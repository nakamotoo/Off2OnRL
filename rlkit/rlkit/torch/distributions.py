...
class ParallelTanhNormal(Distribution):
    """
    Represent distribution of X where
        X ~ tanh(Z)
        Z ~ N(mean, std)
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        ...
        :return:
        """
        log_prob = self.normal.log_prob(pre_tanh_value).sum(dim=-1) # (5, 256)
    ...
    def sample(self):
        # Modified to give mean action
        value, pre_tanh_value = self.rsample_with_pretanh()
        return value.detach().mean(0)
